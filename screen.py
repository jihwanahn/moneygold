...
def screening():
    ...
    for each in items:
        each["score"] = ACTION_SCORE.get(each["action"], -1)
    ...
    for each in items:
        ticker = each["ticker"]
        visited_list.append(ticker)
        last_advice_date = each["last_advice_date"]
        days = days_between(last_advice_date, screener.biz_date)
        refresh_time = ACTION_REFRESH_TIME.get(each["action"], 30)
        if days >= refresh_time:
            tracking_list.append(ticker)

    user_interest = load_user_interest()

    for each in user_interest:
        if each["ticker"] in tracking_list:
            tracking_list.remove(each["ticker"])

    tracking_list = tracking_list[:7]
    exploration = 5

    print("Screening")
    quants = screener.screen(tracking_list, visited_list, exploration)
    ...
    for each in detailed_quants:
        ...
        if os.path.exists(advice_file_path):
            advice = load_text_file(advice_file_path)
        else:
            advice = advisor.get_advice(each)

        try:
            advice_json = json.loads(advice)

            action = advice_json["action"]
            if ticker not in visited_list:
                items.append({
                    "ticker": ticker,
                    "last_advice_date": screener.biz_date,
                    "action": action
                })
            else:
                history_update[ticker] = { "action": action }
        except Exception as e:
            print(f"Manual Adjustment Required : {ticker}")

    for each in items:
        if each["ticker"] in history_update:
            each["last_advice_date"] = screener.biz_date
            each["action"] = action
    ...
    save_screening_history(new_history)
